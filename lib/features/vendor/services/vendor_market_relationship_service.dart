...
import 'package:hipop/features/shared/services/email_service.dart';
...
class VendorMarketRelationshipService {
  ...
  static Future<String> submitMarketPermissionRequest({
    ...
    try {
      ...
      // Send notification email to support (since markets don't have organizerId field yet)
      try {
        final market = await MarketService.getMarket(marketId);
        if (market != null) {
          await EmailService.sendSupportNotification(
            subject: 'New Vendor Application: ${market.name}',
            message: 'Vendor ${vendorProfile.businessName ?? vendorProfile.displayName} (${vendorProfile.email}) has applied to join market "${market.name}".',
            userEmail: vendorProfile.email,
            metadata: {
              'marketId': marketId,
              'marketName': market.name,
              'vendorId': vendorId,
              'vendorEmail': vendorProfile.email,
              'applicationType': 'market_permission',
            },
          );
        }
      } catch (e) {
      ...
      try {
        ...
        if (market != null) {
          await EmailService.sendVendorInvitation(
            relationship: relationship.copyWith(id: docRef.id),
            market: market,
            customMessage: notes,
          );
        }
