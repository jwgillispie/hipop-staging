...
interface SubscriptionData {
  userId: string;
  userType: 'shopper' | 'vendor' | 'market_organizer';
  tier: 'free' | 'shopperPro' | 'vendorPro' | 'marketOrganizerPro' | 'enterprise';
...
export class SubscriptionValidator {
  private static readonly logger = functions.logger;
  
  /**
   * Validate subscription creation request
   */
  static async validateSubscriptionCreation(data: any, context: functions.https.CallableContext): Promise<ValidationResult> {
    ...
    const sanitizedData: Partial<SubscriptionData> = {};
    ...
    try {
      ...
      if (errors.length === 0) {
        const businessRulesResult = await this.validateBusinessRules(sanitizedData as SubscriptionData, context);
   ...
   */
  static async validateFeatureAccess(data: any, context: functions.https.CallableContext): Promise<ValidationResult> {
   ...
   */
  private static validateUserId(userId: any, authUserId: string): ValidationResult {
    const errors: ValidationError[] = [];
   ...
   * Validate user type
   */
  private static validateUserType(userType: any): ValidationResult {
    const errors: ValidationError[] = [];
   ...
   * Validate subscription tier
   */
  private static validateSubscriptionTier(tier: any): ValidationResult {
    const errors: ValidationError[] = [];
   ...
   */
  private static validateStripeCustomerId(customerId: any): ValidationResult {
    const errors: ValidationError[] = [];
   ...
   */
  private static validateStripePriceId(priceId: any): ValidationResult {
    const errors: ValidationError[] = [];
   ...
   */
  private static validateStripeSubscriptionId(subscriptionId: any): ValidationResult {
    const errors: ValidationError[] = [];
   ...
   * Validate feature name
   */
  private static validateFeatureName(featureName: any): ValidationResult {
    const errors: ValidationError[] = [];
   ...
   * Validate metadata
   */
  private static validateMetadata(metadata: any): ValidationResult {
    const errors: ValidationError[] = [];
   ...
   */
  private static async validateBusinessRules(data: SubscriptionData, context: functions.https.CallableContext): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
   ...
   */
  private static async checkRateLimit(userId: string, operation: string): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
   ...
   */
  private static containsSuspiciousChars(input: string): boolean {
   ...
   */
  private static sanitizeMetadataValue(value: any): any {
   ...
   */
  private static isValidTierForUserType(tier: string, userType: string): boolean {
 ...
 */
export const validateSubscriptionCreation = functions.https.onCall(async (data, context) => {
  return SubscriptionValidator.validateSubscriptionCreation(data, context);
});

export const validateFeatureAccess = functions.https.onCall(async (data, context) => {
